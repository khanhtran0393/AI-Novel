...
import { AiNovelPackBootstrap } from '@components/shared/AiNovelPackBootstrap'
...
import { AppUpdater } from '@components/settings/AppUpdater'
...
export function App() {
  ...
  return (
    <>
      ...
      <AiNovelPackBootstrap />
      <FirstRunSetup />
      <AppUpdater />
      {view === 'home' ? <HomeScreen /> : <Editor />}
      <Suspense fallback={null}>{voiceLoaded.current && <VoiceStudioPanel />}</Suspense>
    </>
  )
}
