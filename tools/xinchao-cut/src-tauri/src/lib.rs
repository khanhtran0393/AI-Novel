...
#[derive(Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
struct AiNovelPackPayload {
    pack_root: String,
    manifest_path: String,
    name: String,
    aspect: String,
    media: Vec<AiNovelPackClip>,
}

#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct AiNovelPackManifest {
    version: u32,
    source: String,
    editor: String,
    name: String,
    aspect: String,
    suggested_timeline: Vec<AiNovelPackManifestClip>,
}
...
/// One-shot queue populated from the install-owned desktop launch arguments.
/// The frontend consumes it after its stores are ready. A second launch can
/// replace the pending value through the single-instance callback.
struct AiNovelPackQueue {
    pending: Mutex<Option<Result<AiNovelPackPayload, String>>>,
    active: Mutex<Option<AiNovelPackPayload>>,
}

#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct AiNovelPackImportReport {
    pack_root: String,
    project_id: String,
    media_count: usize,
    clip_count: usize,
...
fn load_ainovel_pack(pack_root: &Path) -> Result<AiNovelPackPayload, String> {
    let canonical_root = pack_root
        .canonicalize()
        .map_err(|error| format!("Không mở được gói AI Novel: {error}"))?;
    if !canonical_root.is_dir() {
        return Err("Gói AI Novel không phải thư mục".to_string());
    }

    let manifest_path = canonical_root.join("ainovel-xinchao-pack.json");
    let canonical_manifest = manifest_path
        .canonicalize()
        .map_err(|error| format!("Thiếu ainovel-xinchao-pack.json: {error}"))?;
    if !canonical_manifest.starts_with(&canonical_root) {
        return Err("Manifest nằm ngoài gói AI Novel".to_string());
    }
    let manifest_text = std::fs::read_to_string(&canonical_manifest)
        .map_err(|error| format!("Không đọc được manifest AI Novel: {error}"))?;
    ...
    if !matches!(
        manifest.aspect.as_str(),
        "16:9" | "9:16" | "1:1" | "4:5" | "4:3" | "21:9"
    ) {
        return Err(format!(
            "Tỷ lệ project không được hỗ trợ: {}",
            manifest.aspect
        ));
    }
    if manifest.suggested_timeline.is_empty() {
        return Err("Manifest không có clip media thật".to_string());
    }

    let mut media = Vec::with_capacity(manifest.suggested_timeline.len());
    for clip in manifest.suggested_timeline {
        if !matches!(clip.kind.as_str(), "video" | "image" | "audio") {
            return Err(format!("Loại media không hợp lệ: {}", clip.kind));
        }
        if !clip.start_sec.is_finite()
            || clip.start_sec < 0.0
            || !clip.duration_sec.is_finite()
            || clip.duration_sec <= 0.0
        {
            return Err(format!("Timeline không hợp lệ tại media {}", clip.key));
        }
        let absolute_path = canonical_root
            .join(&clip.path)
            .canonicalize()
            .map_err(|error| format!("Media {} không tồn tại: {error}", clip.key))?;
    ...
    Ok(AiNovelPackPayload {
        pack_root: canonical_root.to_string_lossy().into_owned(),
...
fn pending_ainovel_pack_from_args(args: &[String]) -> Option<Result<AiNovelPackPayload, String>> {
    let pack_root = args
        .windows(2)
        .find(|pair| pair[0] == "--ainovel-pack")
        .map(|pair| PathBuf::from(&pair[1]))?;
    Some(load_ainovel_pack(&pack_root))
}

#[tauri::command]
fn take_ainovel_pack(
    queue: tauri::State<'_, AiNovelPackQueue>,
) -> Result<Option<AiNovelPackPayload>, String> {
    let pending = queue
        .pending
        .lock()
        .map_err(|_| "Không khóa được hàng đợi AI Novel".to_string())?
        .take();
    match pending {
        Some(Ok(payload)) => {
            *queue
                .active
                .lock()
                .map_err(|_| "Không khóa được project AI Novel đang nạp".to_string())? =
                Some(payload.clone());
            Ok(Some(payload))
        }
...
#[tauri::command]
fn report_ainovel_pack_import(
    queue: tauri::State<'_, AiNovelPackQueue>,
    report: AiNovelPackImportReport,
    ...
    let active = queue
        ...
        .map_err(|_| "Không khóa được project AI Novel đang nạp".to_string())?
        .clone()
        .ok_or_else(|| "Không có gói AI Novel đang hoạt động".to_string())?;
    let reported_root = PathBuf::from(&report.pack_root)
        .canonicalize()
        .map_err(|error| format!("Không xác minh được gói vừa nhập: {error}"))?;
    let active_root = PathBuf::from(&active.pack_root);
    if reported_root != active_root {
        return Err("Project báo cáo sai gói AI Novel đang hoạt động".to_string());
    }
    ...
    let receipt_path = active_root.join("ainovel-xinchao-import-receipt.json");
    let receipt = serde_json::json!({
        "version": 1,
        "source": "xinchao-cut-runtime",
        "packRoot": active.pack_root,
        "manifestPath": active.manifest_path,
        ...
        "importedAt": std::time::SystemTime::now()
            ...
            .map_err(|error| format!("Đồng hồ hệ thống không hợp lệ: {error}"))?
            .as_millis(),
    });
    std::fs::write(
        &receipt_path,
        serde_json::to_vec_pretty(&receipt)
            .map_err(|error| format!("Không tạo được receipt project: {error}"))?,
    )
    .map_err(|error| format!("Không ghi được receipt project: {error}"))?;
    Ok(receipt_path.to_string_lossy().into_owned())
}
...
const MEDIA_EXTENSIONS: &[&str] = &[
    ...
    "ogg", "opus", "wma", "png", "jpg", "jpeg", "webp", "gif", "bmp", "avif",
];
...
mod media_extension_tests {
    ...
    #[test]
    fn accepts_the_same_image_extensions_as_the_frontend_importer() {
        for extension in ["png", "jpg", "jpeg", "webp", "gif", "bmp", "avif"] {
            assert!(is_media_file(&PathBuf::from(format!(
                r"D:\media\frame.{extension}"
            ))));
        }
    }
    ...
    fn rejects_non_media_files() {
        assert!(!is_media_file(&PathBuf::from(
            r"D:\media\ainovel-xinchao-pack.json"
        )));
    }
}
...
pub fn run() {
    ...
    let startup_pack = pending_ainovel_pack_from_args(&startup_args);
    tauri::Builder::default()
        .plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            ...
            if let Some(pack) = pending_ainovel_pack_from_args(&args) {
                if let Ok(mut pending) = app.state::<AiNovelPackQueue>().pending.lock() {
                    *pending = Some(pack);
                }
                let _ = app.emit("ainovel-pack-available", ());
            }
        ...
        .manage(AiNovelPackQueue {
            pending: Mutex::new(startup_pack),
