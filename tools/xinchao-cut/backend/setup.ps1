...
function Find-Python311 {
    if (Get-Command py -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -eq 0 -and "$version" -match "3\.11") {
            try {
                $null = & py -3.11 -c "import encodings" 2>&1
    ...
    $uvPy = Get-ChildItem -Path "$env:APPDATA\uv\python\cpython-3.11*\python.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($uvPy) {
        return @{ Exe = $uvPy.FullName; Prefix = @() }
    }
