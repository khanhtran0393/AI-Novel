...
// ─── XOAY captcha (như G-Labs extension): xoá cookie labs.google + reload tab captcha → RESET điểm
// reCAPTCHA (điểm "unusual activity" phần lớn theo cookie/phiên). Gen dùng Bearer nên clear cookie
// KHÔNG hỏng gen (đúng mô hình guest: tab chỉ mint captcha, auth gen bằng token account). Gọi sau N
// token HOẶC khi gặp UNUSUAL_ACTIVITY → lần thử lại có phiên tươi, ít bị chặn khi chạy lô lớn. ───
let _capTokenCount = 0;
const MAX_CAP_PER_SESSION = 30;
let _capRefreshing = null;
async function refreshCaptcha(why) {
  if (_capRefreshing) return _capRefreshing;   // đang refresh → gộp chung, không reload chồng
  _capRefreshing = (async () => {
    try {
      const cookies = await chrome.cookies.getAll({ domain: 'labs.google' });
      for (const c of cookies) { try { await chrome.cookies.remove({ url: `https://${c.domain.replace(/^\./, '')}${c.path}`, name: c.name }); } catch {} }
      let tabId = await _flowTab();
      if (tabId == null) { const t = await chrome.tabs.query({ url: FLOW_TAB_MATCH }); tabId = t[0]?.id; }
      if (tabId != null) { try { await chrome.tabs.reload(tabId); } catch {} await sleep(4500); }   // chờ tab + grecaptcha nạp lại
      _capTokenCount = 0;
    } catch {}
    finally { _capRefreshing = null; }
  })();
  return _capRefreshing;
}
...
async function apiRequestWithCaptcha(url, body, action, token) {
  ...
  try {
    ...
    if (err) {
      // reCAPTCHA/unusual-activity → xoay captcha (không await để trả lỗi nhanh; retry sau dùng tab tươi).
      if (/UNUSUAL_ACTIVITY|reCAPTCHA|CAPTCHA evaluation/i.test(err)) refreshCaptcha('unusual-activity');
      return { error: err, status: resp.status };
    }
    if (++_capTokenCount >= MAX_CAP_PER_SESSION) refreshCaptcha('đủ ' + MAX_CAP_PER_SESSION + ' token');   // proactive: reset trước khi bị nghi
