...
export default function AINovelDashboard() {
  ...
  const handleDownloadAll = () => {
    window.open('/api/ainovel/download-all', '_blank');
  };
  ...
  const handleSyncAllToWorkspace = async () => {
    ...
    try {
      ...
      alert(`✅ Đã đồng bộ ${ids.length} chương engine → workspace Script.`);
    } catch (err) {
      alert(`Lỗi sync: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-zinc-400">
              ...
              {capsHint ? (
                <span className="ml-2 text-[11px] text-emerald-500/90 font-mono">{capsHint}</span>
              ) : null}
            </p>
            {lastError ? (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {status === 'running' ? (
              ...
              <>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => void handleStartEngine('start')}
                ...
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => void handleStartEngine('resume')}
            ...
            <button
              type="button"
              disabled={busy}
              onClick={() => void handleSyncAllToWorkspace()}
