...
def _resolve_sscronet() -> str:
    """Prefer config path; else search CapCut Apps under LOCALAPPDATA."""
    if SSCRONET_DLL and os.path.isfile(SSCRONET_DLL):
        return SSCRONET_DLL
    local = os.environ.get("LOCALAPPDATA") or os.path.join(
        os.path.expanduser("~"), "AppData", "Local"
    )
    roots = [
        os.path.join(local, "CapCut", "Apps"),
        os.path.join(local, "JianyingPro", "Apps"),
        os.environ.get("AINOVEL_CAPCUT_APPS") or "",
    ]
    for root in roots:
        ...
        try:
            ...
            continue
        for v in versions:
            p = os.path.join(root, v, "sscronet.dll")
            if os.path.isfile(p):
                return p
    raise FileNotFoundError(
        "sscronet.dll not found. Install CapCut Desktop (PC) so "
        "%LOCALAPPDATA%\\CapCut\\Apps\\<ver>\\sscronet.dll exists."
    )
...
def _get_default_client() -> CronetClient:
    ...
    if _default_client is None:
        ...
        # Always re-resolve DLL (config may be empty or stale path)
        cfg["sscronet_dll"] = _resolve_sscronet()
        _default_client = CronetClient(**cfg)
