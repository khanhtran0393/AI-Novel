...
// Define artifact and screenshot directories for the current conversation
const artifactDir = 'C:\\Users\\Khanh\\.gemini\\antigravity\\brain\\eb07f9d2-4b9a-4c98-9cd1-af3e1a997c41';
...
async function main() {
  ...
  console.log('[Browser]: Đang khởi chạy Puppeteer ở chế độ HEADLESS: FALSE (Xem trực tiếp trên màn hình)...');
  const browser = await puppeteer.launch({
    headless: false, // Run in headed mode so the user can watch the browser run live
    slowMo: 60,     // Delay between browser actions for a smoother experience
  ...
  try {
    ...
    try {
      ...
      if (studioTabBtn) {
        ...
        if (genPromptStudioBtn) {
          ...
          for (let i = 0; i < 50; i++) {
            ...
            if (prompts && prompts.length > 0) {
              console.log(`🎉 Đã sinh xong ${prompts.length} visual prompts ở giây thứ ${(i + 1) * 1.5}s!`);
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
        ...
        console.log('Đang chờ nút "Gen ảnh" xuất hiện và nhấp...');
        await new Promise(r => setTimeout(r, 3000)); // Sleep 3s to let DOM re-render
        const genSceneImgBtn = await page.waitForSelector('button ::-p-text(Gen ảnh)', { timeout: 15000 });
        if (genSceneImgBtn) {
